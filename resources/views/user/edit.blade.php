    ...
    <main class="w-auto h-screen p-32 flex justify-center items-center">
        <section>
            <div
                ...
                <form method="POST" action="{{ route('admin.user.update', $user->id) }}" class="grid place-content-center gap-8">
                    @csrf
                    @method('patch')
                    <div>
                        <label for="name" class="mb-3 font-normal">Имя</label>
                        <input id="name" class="block mt-1 rounded-md bg-gray-400 dark:bg-gray-800" type="text"
                            name="name" value="{{ $user->name }}" required>
                    </div>
                    <div>
                        <label for="email" class="mb-3 font-normal text-gray-700 dark:text-gray-400">Почта</label>
                        <input id="email" class="block mt-1 rounded-md bg-gray-400 dark:bg-gray-800" type="email"
                            name="email" value="{{ $user->email }}" required>
                    </div>
                    <div>
                        <label for="password" class="mb-3 font-normal text-gray-700 dark:text-gray-400">Пароль</label>
                        <input id="password" class="block mt-1 rounded-md bg-gray-400 dark:bg-gray-800" type="password"
                    ...
                    <div class="flex-column justify-center">
                        ...
                        <div>
                            <select
                                class="w-full rounded-md bg-gray-400 dark:bg-gray-800 focus:border-inherit focus:ring-0"
                                name="role_id" id="role_id"
                                class="bg-gray-400 dark:bg-gray-700 text-gray-700 dark:text-gray-400">
                                @foreach ($roles as $role)
                                    <option
                                    
                                    {{$role->id === $user->role->id ? 'selected' : ''}}
                                    
                                    class="text-gray-700 dark:text-gray-400" value="{{ $role->id }}">
                                        {{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
